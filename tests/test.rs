...
tests! {
    ...
    lvar0: ("int main() { a = 1; a;} ", 1),
    lvar1: ("int main() { b = 1 + 3 * (4 - 2); b;} ", 7),
    lvar2: ("int main() { foo = 10 + 4; foo;} ", 14),
    lvar3: ("int main() { a = 1; b = 2; a + b;} ", 3),
    return0: ("int main() { return 1;} ", 1),
    return1: ("int main() { return (3 + 1) *  10;} ", 40),
    return2: ("int main() { a = 4 + 5; return a + 1;} ", 10),
    ...
    ifelse2: ("int main() { a = 4; if (a == 4) return 1; else return 5;} ", 1),
    ifelse3: ("int main() { boo = 19; if (boo == 1) return 10; else return 2;} ", 2),
    while0: ("int main() { a = 0; while (a != 10) a = a + 1; return a;} ", 10),
    while1: ("int main() { a = 0; while (a != 10) if (a == 3) return a; else a = a + 1; return a;} ", 3),
    for0: ("int main() { a = 0; for (i = 0; i < 10; i = i + 1) a = a + 1; return a;} ", 10),
    for1: ("int main() { a = 0; for (;a < 13;) a = a + 1; return a;} ", 13),
    block0: ("int main() { a = 0; while (a != 10) { a = a + 1; } return a;} ", 10),
    block1: ("int main() { for (a = 0; a <= 14; a = a +1) {} return a;} ", 15),
    block2: ("int main() { a = 1 * 5; if (a == 5) {a =7; return a;} else {return 10; return 11;} }", 7),
    addr0: ("int main() { x = 3; y = 5; z = &y + 8; return *z; } ", 3),
